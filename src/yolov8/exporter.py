...
def export_handler(yolo:YOLO, task_yolo, dataset_folder, args_export, args_training, args_task):
    print(f'\n[Export Model]')

    with open(os.path.join(dataset_folder, "data.yaml"), "r") as f:
        data_yaml = yaml.load(f, Loader=SafeLoader)

    upload_to_clearml(task_yolo, args_training, args_task, data_yaml, "raw-best", yolo.trainer.best)
    upload_to_clearml(task_yolo, args_training, args_task, data_yaml, "raw-last", yolo.trainer.last)
    for format, is_use in args_export["format"].items():
        try:
            ...
            print(path_model)
            upload_to_clearml(task_yolo, args_training, args_task, data_yaml, format, path_model)

        except Exception as e:
            print(f"FAILED EXPORT MODEL {format}",e)

def upload_to_clearml(task_yolo, args_training, args_task, data_yaml, format, path_model):
